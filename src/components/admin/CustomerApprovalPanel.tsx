...
import { compressImage, safeJson } from "@/lib/image-compress";
...
export function CustomerApprovalPanel({
  ...
  const { getRootProps, getInputProps, isDragActive } = useDropzone({
    ...
    onDropRejected: () => setError("Dosya reddedildi. Sadece resim dosyaları (JPG, PNG, WEBP), maks. 25MB kabul edilir."),
  });
  ...
  const handleSendForApproval = async () => {
    ...
    try {
      ...
      if (uploadRes.status === 413) {
        throw new Error("Yüklediğiniz görsel çok büyük, lütfen boyutunu düşürün.");
      }
      const uploadJson = await safeJson(uploadRes);
      if (!uploadRes.ok || !uploadJson?.url) {
        throw new Error((uploadJson?.error as string) ?? "Görsel yüklenemedi.");
      }
      ...
      const approvalJson = await safeJson(approvalRes);
      if (!approvalRes.ok || !approvalJson) {
        throw new Error((approvalJson?.error as string) ?? "Onay talebi gönderilemedi.");
      }

      setSmsFeedback({ sent: Boolean(approvalJson.smsSent), error: approvalJson.smsError as string | undefined });
      onUpdated(approvalJson.order as Order);
      setFile(null);
      setRetrying(false);
    } catch (err) {
          ...
          >
            ...
            {previewUrl ? (
              ...
              <>
                ...
                <p className="text-[11px] text-[#a09890] mt-1">PNG, JPG, WEBP — büyük fotoğraflar otomatik sıkıştırılır</p>
              </>
            )}
          </div>
          ...
          >
            {sending ? (
              <>
                ...
                {compressing ? "Sıkıştırılıyor..." : "Gönderiliyor..."}
              </>
            ) : (
              "Onaya Gönder"
            )}
