  ...
  const btn = (
    ...
    <button
      ...
      className={`p-1.5 rounded text-[13px] transition-colors ${
        active ? "bg-[#1d3435] text-white" : "text-[#545454] hover:bg-[#f0f0f0]"
      }`}
    >
      {content}
    </button>
...
function ImageDropzone({ images, onAdd, onRemove }: {
  ...
  const { getRootProps, getInputProps, isDragActive } = useDropzone({
    ...
    multiple: true,
    maxSize: 5 * 1024 * 1024,
    onDropRejected: () => setUploadError("Dosya reddedildi. Sadece resim dosyaları (JPG, PNG, WEBP) desteklenir. RAR/ZIP arşiv içinden değil, çıkartılmış dosyaları yükleyin."),
    onError: () => setUploadError("Dosyaya erişilemedi. Lütfen RAR/ZIP arşivini önce çıkartın, sonra fotoğrafı sürükleyin."),
  });
    ...
    <div className="space-y-3">
      <div
        ...
        className={`border-2 border-dashed rounded-lg p-6 sm:p-8 text-center cursor-pointer transition-all ${
          isDragActive
            ? "border-[#3d7b74] bg-[#3d7b74]/5"
            : "border-[#e8e8e8] hover:border-[#3d7b74] hover:bg-[#f9f9f9]"
        }`}
      >
        <input {...getInputProps()} />
        {uploading ? (
          ...
          <div className="flex flex-col items-center gap-2 text-[#999]">
            ...
            <p className="text-[12px]">PNG, JPG, WEBP — maks. 5 MB</p>
          </div>
        )}
      </div>
        ...
        <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-6 gap-3">
          {images.map((url, i) => (
            <div key={i} className="relative group aspect-square rounded-lg overflow-hidden border border-[#e8e8e8]">
              ...
              {i === 0 && (
                <span className="absolute top-1 left-1 text-[9px] bg-[#1d3435] text-white px-1.5 py-0.5 rounded font-bold">
                  ANA
                </span>
              )}
              <button
...
const inputErrCls = "w-full border border-red-300 rounded-md px-3 py-2 text-[13px] focus:outline-none focus:border-red-400 focus:ring-2 focus:ring-red-400/10 transition-all bg-white";
...
export default function NewProductPage() {
  ...
  const onSubmit: SubmitHandler<ProductFormValues> = async (values) => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        throw new Error(e.error ?? "Kayıt hatası");
      }
  ...
  return (
    <div className="max-w-4xl space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center gap-3">
        ...
        <div className="flex gap-2 flex-shrink-0">
          ...
          <button
            ...
            disabled={saving}
            className="px-4 sm:px-5 py-2 bg-[#1d3435] text-white rounded-md text-[13px] font-bold hover:bg-[#2a4a4b] transition-colors disabled:opacity-60 flex items-center gap-2"
          >
            {saving ? (
              <>
      ...
      {/* ── Zod validation summary (field errors at top) ── */}
        ...
        <div className="bg-red-50 border border-red-200 rounded-lg px-4 py-3 text-[13px] text-red-700">
          ...
          <ul className="list-disc list-inside space-y-0.5">
            {Object.entries(errors).map(([key, err]) =>
              "message" in err ? (
                <li key={key}>{err.message as string}</li>
              ) : null
            )}
          </ul>
        </div>
          ...
          <div>
            ...
            <input
              ...
              onChange={(e) => {
                ...
                if (!watchedSeoTitle) {
                  setValue("seoTitle", e.target.value, { shouldValidate: false });
                }
              }}
            />
      ...
      <Section title="Kategori & Alt Kategori">
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          <div>
            ...
            >
              <option value="">
                {subCategories.length === 0 ? "Önce ana kategori seçin" : "Seçin..."}
              </option>
              {subCategories.map((s) => (
        ...
        {watchedCatSlug && (
          <p className="text-[12px] text-[#3d7b74] mt-3 font-medium">
            📍 {selectedCat?.name}
            {watch("subCategoryName") ? ` › ${watch("subCategoryName")}` : ""}
          </p>
        )}

        {/* Ek Kategoriler */}
        <div className="border-t border-[#ebebeb] pt-4 mt-2 space-y-3">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-[11px] font-bold uppercase tracking-widest text-[#1d3435]">Ek Kategoriler</p>
              <p className="text-[11px] text-[#999] mt-0.5">Bu ürünü birden fazla kategoride göstermek için ekleyin.</p>
            </div>
            <button type="button"
              onClick={() => setExtraCats((p) => [...p, { categorySlug: "", subCategorySlug: "" }])}
              className="text-[12px] font-bold text-[#3d7b74] hover:text-[#1d3435] transition-colors whitespace-nowrap">
              + Kategori Ekle
            </button>
          </div>
          {extraCats.length === 0 && (
            <p className="text-[11px] text-[#bbb]">Ek kategori yok — bu ürün yalnızca ana kategorisinde görünür.</p>
          )}
          {extraCats.map((ec, idx) => {
            ...
            return (
              <div key={idx} className="flex items-center gap-2">
                ...
                <select className={inputCls} value={ec.subCategorySlug ?? ""}
                ...
                <button type="button"
                  onClick={() => setExtraCats((p) => p.filter((_, i) => i !== idx))}
      ...
      <Section title="Fiyat & Stok">
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          ...
          <div>
            ...
            />
            {discountPct > 0 && (
              <p className="text-[11px] text-green-600 mt-1 font-semibold">%{discountPct} indirim uygulanacak</p>
            )}
          ...
          <div>
            ...
            {Number(watchedStock) === 0 && (
              <p className="text-[11px] text-red-500 mt-1">Stokta yok olarak görünür</p>
            )}
            {Number(watchedStock) > 0 && Number(watchedStock) < 5 && (
              <p className="text-[11px] text-orange-500 mt-1">Düşük stok uyarısı</p>
            )}
      ...
      <Section title="Bakım Talimatları" badge="Opsiyonel">
        <div>
          <p className="text-[12px] text-[#999] mb-3">
            Bu ürüne özel bakım talimatları. Boş bırakılırsa varsayılan talimatlar gösterilir. Her satır ayrı madde olarak listelenir.
          </p>
          <textarea
            rows={5}
      ...
      <Section title="Varyant Yönetimi" badge="Opsiyonel">
        <p className="text-[12px] text-[#999] mb-4">
          Renk, beden veya boyut gibi seçenekler varsa ekleyin. Her varyant için ayrı fiyat ve stok girebilirsiniz.
        </p>
        ...
        {variantFields.length > 0 && (
          <div className="space-y-3 mb-4">
            ...
            <div className="hidden sm:grid grid-cols-12 gap-2 text-[10px] font-bold uppercase tracking-widest text-[#999] px-1">
              <span className="col-span-5">Varyant Adı (Ör: Kırmızı XL)</span>
            ...
            {variantFields.map((field, index) => (
              <div key={field.id} className="flex flex-col sm:grid sm:grid-cols-12 gap-2 items-start sm:items-center bg-[#fafafa] sm:bg-transparent rounded-lg sm:rounded-none p-3 sm:p-0 border border-[#f0f0f0] sm:border-0">
                <div className="w-full sm:col-span-5">
                  <label className="sm:hidden text-[10px] font-bold text-[#999] uppercase tracking-widest mb-1 block">Varyant Adı</label>
                  <input
                    type="text"
                    placeholder="Ör: Kırmızı / L"
                    {...register(`variants.${index}.label`)}
                    className={errors.variants?.[index]?.label ? inputErrCls : inputCls}
                  />
                  <FieldError msg={errors.variants?.[index]?.label?.message} />
                </div>
                <div className="w-full sm:col-span-3">
                  <label className="sm:hidden text-[10px] font-bold text-[#999] uppercase tracking-widest mb-1 block">Fiyat (₺)</label>
                  <input
                    type="number"
                    placeholder="450"
                  ...
                  />
                  <FieldError msg={errors.variants?.[index]?.price?.message} />
                </div>
                <div className="w-full sm:col-span-3">
                  <label className="sm:hidden text-[10px] font-bold text-[#999] uppercase tracking-widest mb-1 block">Stok</label>
                  <input
                    type="number"
                    placeholder="0"
        ...
        <div className="space-y-4">
          <div>
            ...
            <input
              type="text"
              {...register("seoTitle")}
              placeholder={watchedName || "Ürün adı otomatik kullanılır"}
              className={inputCls}
            />
            <p className="text-[11px] text-[#999] mt-1">
              {(watchedSeoTitle ?? "").length}/60 karakter — ideal uzunluk 50-60 karakter
            </p>
          </div>
          <div>
            ...
            <textarea
              ...
              placeholder="Arama motorlarında görünecek kısa açıklama (150-160 karakter)"
              className={`${inputCls} resize-none`}
            />
            <p className="text-[11px] text-[#999] mt-1">
              {(watchedSeoDesc ?? "").length}/160 karakter
            </p>
          </div>
          <div className="border border-[#e8e8e8] rounded-md p-3 bg-[#f9f9f9]">
            ...
            <p className="text-[11px] text-green-700">
              bestcicekcilik.com › urun › {watchedSlug || "urun-slug"}
            </p>
