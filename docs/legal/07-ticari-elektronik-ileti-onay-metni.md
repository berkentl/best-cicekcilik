...
# EK — UYGULAMA GEREKLİLİKLERİ

> *Bu bölüm hukuki metnin parçası değildir; onay mekanizmasının mevzuata uygun kurulabilmesi için karşılanması gereken koşulları içerir.*

## Zorunlu kurallar

1. **Kutucuk varsayılan olarak işaretsiz gelmelidir.** Önceden işaretli kutucukla alınan onay geçersizdir.

2. **Ticari ileti onayı, KVKK açık rızası ve sözleşme onaylarından ayrı kutucukta alınmalıdır.** Üç ayrı hukuki rejime tabidir; tek kutucukta toplanamazlar.

3. **Onay, sipariş verme veya üyelik oluşturmanın önkoşulu yapılamaz.**

4. **Onay alındıktan sonra İYS'ye yüklenmelidir.** Yükleme yapılmadan gönderilen ileti, onay bulunsa dahi mevzuata aykırıdır ve idari para cezası doğurur.

5. **Her iletide ret imkânı bulunmalıdır** — SMS'lerde ret kodu, e-postalarda abonelikten çık bağlantısı.

6. **Gönderim saatleri 08:00–23:00 aralığında sınırlandırılmalıdır.**

## Önerilen kutucuk metni

*(varsayılan: işaretsiz)*

> ☐ Kampanya, indirim ve özel gün duyurularının tarafıma **SMS** ve **e-posta** ile gönderilmesine, **Ticari Elektronik İleti Onay Metni**'ni okuyarak onay veriyorum.

Kanal bazlı ayrım isteniyorsa iki ayrı kutucuk kullanılabilir:

> ☐ E-posta ile ticari ileti almak istiyorum
> ☐ SMS ile ticari ileti almak istiyorum

## Saklanması gereken onay kaydı

| Alan | Açıklama |
|---|---|
| Onay konusu | `ticari_ileti_sms` / `ticari_ileti_eposta` |
| Durum | Onay verildi / reddedildi |
| Tarih-saat | İşlemin gerçekleştiği an |
| Kanal | Üyelik formu / ödeme adımı / hesap ayarları / İYS |
| IP adresi | İşlemin yapıldığı IP |
| Metin sürümü | Onaylanan metnin sürümü |
| **İYS yükleme durumu** | Yüklendi / bekliyor / hata |
| **İYS yükleme tarihi** | İYS'ye işlendiği an |
| Ret tarihi | Varsa ret bildiriminin alındığı an |

**İYS yükleme durumunun ayrı bir alan olarak tutulması zorunludur.** Onay alınmış fakat İYS'ye yüklenmemiş kayıtlar, denetimde onaysız gönderim gibi değerlendirilir; bu nedenle yüklenmeyen kayıtların tespit edilebilir olması gerekir.

## İYS entegrasyonu hakkında

İYS kaydınız henüz aktif olmadığından, ticari ileti gönderimine **İYS kaydı tamamlanmadan başlanmamalıdır.** Onay kutucukları arayüze şimdiden eklenebilir ve onaylar veri tabanında toplanabilir; ancak fiilî gönderim, kayıt tamamlanıp mevcut onaylar İYS'ye yüklendikten sonra başlatılmalıdır.

NetGSM üzerinden SMS gönderiyorsanız, NetGSM'in İYS entegrasyonu bulunmaktadır; onayların NetGSM paneli üzerinden İYS'ye aktarılması mümkündür. Bu akışın kurulması, ticari SMS gönderimine başlamadan önce tamamlanmalıdır.
